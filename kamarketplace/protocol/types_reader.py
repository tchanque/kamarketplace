class Data:
    ...
    def read(self, bytes_to_read):
        print("Reading %s bytes from %s" % (bytes_to_read, self.remaining))
        result = self.data[self.pos: self.pos + bytes_to_read]
        print("Has read %s" % result)
    ...
    def read_float(self):
        ...
        return float(int.from_bytes(self.read(4), byteorder="big"))
    ...
    def read_varlong(self):
        print("Decoding VarLong from %s" % self)
        return int.from_bytes(self[:8], byteorder='big'), self[8:]
